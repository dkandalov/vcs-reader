...
class SvnRepository {
	...
	def commit(String message, Date date) {
		svn("commit", "-m", message)
		svn("propset", "svn:date", "--revprop", "-r", "HEAD", date.format("yyyy-MM-dd'T'HH:mm:ss.000000'Z'", UTC))
	...
	def dummyCommit(Date date) {
		create("dummy")
		commit("dummy commit otherwise svn log doesn't show commits", date)
	}
